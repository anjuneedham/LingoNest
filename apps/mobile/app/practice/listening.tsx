...
export default function ListeningPractice() {
  ...
  const lessonsQuery = useQuery({
    queryKey: ['skill-lessons', 'listening', languageCode],
    queryFn: async () => {
  ...
  return (
    ...
    >
      <Text variant="title">{t('practice.listening')}</Text>
      <Text variant="small" color="muted" style={{ marginTop: spacing.xs }}>
        {t('practice.listeningBody')}
      </Text>
        ...
        <Animated.View key={lesson.id} entering={FadeInDown.delay(i * 60)}>
          <Card
            ...
            style={{
              ...
              borderLeftWidth: 4,
              borderLeftColor: PRACTICE_MODE_COLORS.listening,
            }}
